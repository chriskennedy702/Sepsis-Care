...
vol as(
  SELECT [Facility ID]
        ...
        ,[City]
        ,[State]
        ,[Score] as 'Emergency Department Volume'	
		,[Sample]
  FROM [Timely and Effective].[dbo].['Timely_and_Effective_Care-Hospi$']  
  Where Score not like '%Not Available%' and [Measure Name] like '%volume%' 
			  and Score like '%high%' and [Facility ID] not like '%NULL%' 
			  or Score like 'very high' and [Facility ID] not like '%NULL%'
	   )
...
SELECT wait.[Facility ID]
	...
	JOIN vol ON six.[Facility ID] = vol.[Facility ID]
